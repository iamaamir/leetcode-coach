...
function getProviderConfig(provider) {
  if (provider === "ollama") {
    const base = document
      .getElementById("ollama-url")
      .value.trim()
      .replace(/\/$/, "");
    const model =
      document.getElementById("ollama-model").value.trim() || "llama3.2";
    return {
      url: base + "/v1",
      model,
  ...
  if (provider === "lmstudio") {
    const base = document
      .getElementById("lmstudio-url")
      .value.trim()
      .replace(/\/$/, "");
    const model =
      document.getElementById("lmstudio-model").value.trim() || undefined;
    return {
      url: base + "/v1",
      model,
      headers: {
...
function startAnalyzingTitle() {
  ...
  titleInterval = setInterval(() => {
    document.title =
      "LeetCode Coach — Analyzing " + frames[i++ % frames.length];
  }, 120);
}
  ...
  try {
    ...
    if (currentProvider === "claude") {
      ...
      if (!cfg.url || cfg.url === "/v1")
        throw new Error(
          "Please configure the Base URL for " +
      ...
      if (!raw)
        throw new Error(
          "Empty response from model. Is a model loaded/running?",
        );
    }
    ...
    if (!jsonMatch)
      throw new Error(
        "Model did not return valid JSON. Try a larger/smarter model.",
      );
...
async function copyText(text) {
  ...
  // `execCommand` is depricated, its here for the fallback
...
function renderOutput(d, lang) {
  ...
  // Helper to generate a section card. Keeps the rendering logic DRY.
  ...
  const sections = [
    ...
    },
    {
      ...
      content: () => {
        const code = d.code || "";
        const idx = renderedCodeSnippets.push(code) - 1;
        return `<div class="code-wrap">
          <div class="code-toolbar">
            <button class="copy-btn" type="button" data-code-index="${idx}" onclick="copyCode(this)">Copy</button>
          </div>
    ...
    },
    {
      ...
      title: "Dry Run",
      content: () => {
        const { description = "", table = [] } = d.dryrun || {};
        let html = `<p class="dry-description">${escHtml(description)}</p>`;
        if (table.length) {
    ...
    },
    {
      ...
      content: () => {
        ...
        return `<div class="complexity-pills">
          <div class="cpill"><div><div class="cpill-label">Time</div>
            <div class="cpill-value">${escHtml(time)}</div>
            <div style="font-size:var(--text-xs);color:var(--muted);margin-top:4px">${escHtml(time_reason)}</div></div>
          </div>
          <div class="cpill"><div><div class="cpill-label">Space</div>
            <div class="cpill-value">${escHtml(space)}</div>
            <div style="font-size:var(--text-xs);color:var(--muted);margin-top:4px">${escHtml(space_reason)}</div></div>
          </div>
        </div>`;
      },
    },
    {
      ...
      content: () => {
        const items = (d.edge_cases || [])
          .map(
            (e) => `
            <div class="edge-item"><div class="edge-bullet">◆</div>
